...
describe('rollup.watch', () => {
		...
		await new Promise((fulfil, reject) => {
			function go(event) {
				...
				if (!next) {
					...
					watcher.once('event', event_1 => {
						if (event_1.code !== next) {
							watcher.close();
							if (event_1.code === 'ERROR') console.log(event_1.error);
							reject(new Error(`Expected ${next} event, got ${event_1.code}`));
						} else {
							go(event_1);
						}
					});
	...
	it('handles closing the watcher during a build', async () => {
		...
		const events = [];
		watcher.on('event', event_1 => events.push(event_1.code));
		return sequence(watcher, [
			'START',
			'BUNDLE_START',
			() => {
